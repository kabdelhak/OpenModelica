...
protected
  ...
  algorithm
    (varData, eqData) := match (varData, eqData)
      ...
      case (BVariable.VAR_DATA_SIM(), BEquation.EQ_DATA_SIM()) algorithm
          ...
          //            1. 2. 3.
          // -----------------------------------
          (replacements, newEquations) := aliasCausalizePre(varData.unknowns, eqData.simulation, kind, "Simulation");
  ...
  algorithm
    (varData, eqData) := match partition.strongComponents
      ...
      case SOME(comps) algorithm
        ...
        //            1. 2. 3.
        // -----------------------------------
        (replacements, comps_to_remove, solved_map) := aliasCausalizePost(comps, Partition.Partition.getKind(partition));
    ...
    (varData, eqData) := match (varData, eqData)
      ...
      case (BVariable.VAR_DATA_SIM(), BEquation.EQ_DATA_SIM())
        algorithm
          ...
          //            1. 2. 3.
          // -----------------------------------
          (replacements, newEquations) := aliasCausalizePre(varData.clocks, eqData.clocked, kind, "Clocked");
          replacements := checkReplacements(replacements, eqData, function updateAuxEquationPre(auxEquations = auxEquations));
  ...
  algorithm
    ...
    replacements        := aliasReplacements(map, kind, context);
  ...
  algorithm
    ...
    replacements  := aliasReplacements(map, kind, "Post Causalization");
